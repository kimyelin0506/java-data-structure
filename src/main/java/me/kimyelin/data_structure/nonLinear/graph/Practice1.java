...
class MyGraphList{
    ...
    public void addEdge(int x, int y){
        this.adjList[x] = new Node(y, this.adjList[x]); //추가 될때마다 앞에다 새로 생성하고 연결하는 방식
        this.adjList[y] = new Node(x, this.adjList[y]);  //끝까지 순회해서 찾는 시간을 아낌
    }
    ...
    public void printAdjacentList(){
        for (int i = 0; i < this.elemCnt; i++) {
            ...
            Node cur = this.adjList[i];  //연결된 수만큼 순회하면서 출력
            while(cur != null){
