...
### 알고리즘-10주차
    1. practice: 최단 경로
        (1) 최단 경로 알고리즘
            -가중 그래프 사으이 두 노드를 연결하는 가장 짧은 경로를 찾는 방법
            -지도 경로 검색, 네트워크 구축에 드는 비용을 최소화하는데 이용
            -최단 경로 알고리즘 종류: 다익스트라, 벨만-포드, 플로이드-워셜
        (2) 다익스트라(Dijkstra)
            -한 출발점에서 목표점까지 최단 경로를 구하는 알고리즘
            -한 노드에서 다른 노드로의최단 경로를 구할 수 있음
            -간선에 음의 가중치가 존재하면 안됨
            -그리디 + DP형태
            -알고리즘 복잡도: O(ElogV) *E: 간선의 수, V: 노드 수
        (2-1) 다익스트라 알고리즘 Flow
            -처음 distance의 정보는 INF로 설정(구현: Integer.MAX_VALUE)
            -출발 지점을 기준으로 인접한 노드들의 가중치를 업데이트
            -다음 노드 선택은 방문한 노드를 제외한 가장 작은 dist를 가진 노드를 선택
            -다음 노드를 선택하면 그 노드까지의 쌓인 가중치를 인접한 노드들의(방문X) dist를 더한 값이
                이전에 저장된 가중치보다 작을 경우 가중치를 업데이트함
            -모든 노드를 방문할때까지 반복
        (3) 벨만-포드(Bellman-Ford)
            -음수 간선이 포함되어 있어도 최단 경로를 구할 수 있음
                -음수 사이클이 있으면 정상 동작 X
            -매번 모든 간선을 확인 -> 다익스트라보다 느림
            -알고리즘 복잠도: O(VE) *V: Value, E: Edge
        (3-1) 벨만 포드 알고리즘 Flow
            -시작 노드부터 인접한 다음 노드들(방향O)의 dist를 업데이트함
            -다른 노드에서 weight와 dist를 더한 값이 더 작을면 dist를 업데이트함
            -마지막 방문을 지나고 v+1번째에서 업데이트정보가 바뀌면 음수 사이클이 발생했다는 의미
        (4) 플로이드-워셜(Floyd-Warshall)
            -모든 노드 간의 최단 경로를 구하는 알고리즘
            -음의 간선이 포함되어 있어도 가능
                -음수 사이클이 있는 경우 정상 동작X
            -알고리즘 복잡도: O(v^3)
        (4-1) 플로이드-워셜 알고리즘 Flow
            -v*v만큼의 업데이트 메모리에 초기값으로 본인을 제외한 모든 값은 INF
            -시작 노드(k)를 시작으로, 노드(k)를 지나가는 노드와(i) 노드(j)의 경우에 값을 업데이트
            -음수 사이클 판단: 벨만-포드 알고리즘과 같음